...
class ConfigPanel:
    ...
    def quick_save_config(self):
        ...
        result = messagebox.askyesno(
            "確認快速保存", 
            f"將以下配置直接保存到 config.py:\n\n{change_list}\n\n這將立即生效並重寫原始配置文件。確定要繼續嗎？"
        )
        ...
        if success:
            ...
            # 寫入config.py文件
            self.write_config_to_file(changed_configs)
            
            # 回調通知父窗口
            if self.on_config_changed:
                self.on_config_changed(changed_configs)
            
            messagebox.showinfo("保存成功", f"配置已直接保存到 config.py\n\n更新了 {len(changed_configs)} 項配置")
        else:
            messagebox.showerror("錯誤", "快速保存配置失敗")
    
    def write_config_to_file(self, changed_configs):
        """將配置寫入config.py文件"""
        try:
            import os
            ...
            print(f"✅ 配置已寫入 {config_file_path}")
            
        except Exception as e:
            print(f"❌ 寫入config.py失敗: {e}")
            messagebox.showerror("錯誤", f"寫入config.py失敗:\n{str(e)}")
